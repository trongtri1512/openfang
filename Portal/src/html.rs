...
:root{--o:#FF5C00;--oh:#e65200;--obg:rgba(255,92,0,.08);--ol:#fff7ed;--bg:#ffffff;--bg2:#F9F8F7;--bg3:#EDECEB;--t:#1A1817;--d:#6B6560;--m:#9ca3af;--b:#D5D2CF;--g:#22c55e;--gb:#f0fdf4;--gt:#15803d;--r:#ef4444;--rb:#fef2f2;--rt:#dc2626;--pb:#faf5ff;--pt:#7c3aed;--bb:#eff6ff;--bt:#2563eb;--sbbg:#EDECEB;--sb-border:rgba(0,0,0,.06);--shadow:rgba(0,0,0,.04);--modal-overlay:rgba(0,0,0,.4);--si-hover:rgba(0,0,0,.04);--si-active-text:#fff;--login-right-bg:#fff;--input-bg:#fff}
.dark{--obg:rgba(255,92,0,.12);--ol:#1a0f05;--bg:#0f0e0d;--bg2:#080706;--bg3:#1a1918;--t:#f5f5f4;--d:#a8a29e;--m:#78716c;--b:rgba(255,255,255,.08);--gb:rgba(34,197,94,.1);--gt:#4ade80;--rb:rgba(239,68,68,.1);--rt:#f87171;--pb:rgba(124,58,237,.1);--pt:#a78bfa;--bb:rgba(37,99,235,.1);--bt:#60a5fa;--sbbg:#0a0908;--sb-border:rgba(255,255,255,.06);--shadow:rgba(0,0,0,.2);--modal-overlay:rgba(0,0,0,.7);--si-hover:rgba(255,255,255,.05);--si-active-text:#0f0e0d;--login-right-bg:#0f0e0d;--input-bg:#1a1918}
...
.login-right{width:480px;display:flex;flex-direction:column;justify-content:center;padding:48px;position:relative;background:var(--login-right-bg)}
...
.sb{width:240px;background:var(--sbbg);border-right:1px solid var(--sb-border);display:flex;flex-direction:column;flex-shrink:0;position:fixed;left:0;top:0;bottom:0;z-index:10}
.sbh{padding:16px 20px;display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--sb-border)}
...
.sbu{padding:12px 20px;font-size:.8rem;color:var(--d);border-bottom:1px solid var(--sb-border);display:flex;align-items:center;justify-content:space-between}
.sbn{flex:1;padding:8px;overflow-y:auto;scrollbar-width:thin;scrollbar-color:var(--m) transparent}
...
.si:hover{background:var(--si-hover);color:var(--t)}
.si.active{background:var(--o);color:var(--si-active-text);font-weight:600}
.si.active svg{stroke:var(--si-active-text)}
.si svg{width:18px;height:18px;flex-shrink:0}
.sbb{padding:8px;border-top:1px solid var(--sb-border)}
...
/* Table */
.dt{width:100%;border-collapse:collapse;font-size:.85rem;background:var(--bg);border:1px solid var(--b);border-radius:12px;overflow:hidden;box-shadow:0 1px 2px var(--shadow)}
...
/* Section Box */
.sbox{background:var(--bg);border:1px solid var(--b);border-radius:12px;padding:20px;margin-bottom:20px;box-shadow:0 1px 2px var(--shadow)}
...
.fg select,.fg input[type=text],.fg input[type=password]{width:100%;padding:10px 14px;border:1px solid var(--b);border-radius:10px;font-size:.85rem;font-family:inherit;color:var(--t);outline:none;background:var(--input-bg)}
...
/* Modal */
.modal-bg{display:none;position:fixed;inset:0;background:var(--modal-overlay);z-index:100;align-items:center;justify-content:center}
.modal-bg.show{display:flex}
.modal{background:var(--bg);border:1px solid var(--b);border-radius:12px;padding:24px;width:440px;max-width:90vw;box-shadow:0 20px 60px var(--shadow)}
.modal h3{font-size:1.1rem;font-weight:700;margin-bottom:16px}
.modal .fg input,.modal .fg select{width:100%;padding:10px 14px;border:1px solid var(--b);border-radius:10px;font-size:.85rem;font-family:inherit;color:var(--t);outline:none;background:var(--input-bg)}
...
/* Warning */
.warn{display:flex;align-items:center;gap:8px;padding:12px 16px;background:var(--ol);border:1px solid var(--obg);border-radius:10px;font-size:.85rem;color:var(--o);margin-bottom:16px}
.theme-toggle{background:none;border:1px solid var(--b);border-radius:8px;padding:4px 8px;cursor:pointer;font-size:.9rem;line-height:1;color:var(--d);transition:all .15s}
...
<div class="dashboard" id="dashView">
  <div class="dl">
    <div class="sb">
      ...
      <div class="sbu" id="sbUser"><span>Admin</span><button class="theme-toggle" id="themeToggle" onclick="toggleTheme()" title="Chuyển sáng/tối">🌙</button></div>
      <div class="sbn">
...
function toggleTheme(){const d=document.documentElement.classList.toggle('dark');localStorage.setItem('portal_theme',d?'dark':'light');document.getElementById('themeToggle').textContent=d?'☀️':'🌙'}
function initTheme(){const t=localStorage.getItem('portal_theme');if(t==='dark'){document.documentElement.classList.add('dark');const b=document.getElementById('themeToggle');if(b)b.textContent='☀️'}}
async function showDash(){document.getElementById('loginView').style.display='none';document.getElementById('dashView').style.display='block';const userSpan=document.querySelector('#sbUser span');if(userSpan)userSpan.textContent=S.display_name||S.email;initTheme();if(S.role==='admin'){document.getElementById('managementLabel').style.display='';document.getElementById('membersNav').style.display='';document.getElementById('usersNav').style.display='';document.getElementById('plansNav').style.display=''}await loadT();showPage('tenants')}
...
(function(){initTheme();const s=localStorage.getItem('ps');if(s){try{S=JSON.parse(s);
